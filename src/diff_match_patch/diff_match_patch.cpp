...
QString Patch::toString()
{
    ...
    QString text;
    text =
        QString("@@ -") + coords1 + QString(" +") + coords2 + QString(" @@\n");
                                           ...
                                           bool checklines, clock_t deadline)
{
...

    {
        ...
        if (i != -1) {
            ...
            const Operation op =
                (text1.length() > text2.length()) ? DELETE : INSERT;
                                         ...
                                         const QStringList &lineArray)
{
    ...
    while (i.hasNext()) {
        ...
        for (int y = 0; y < diff.text.length(); y++) {
            text +=
                lineArray.value(static_cast<ushort>(diff.text[y].unicode()));
        }
        diff.text = text;
    }
}
                                         ...
                                         const QString &text2)
{
    ...
    while (true) {
        ...
        length += found;
        if (found == 0 ||
            text1_trunc.right(length) == text2_trunc.left(length)) {
            best = length;
            length++;
        }
    }
        ...
        if (prevDiff->operation == DELETE && thisDiff->operation == INSERT) {
            ...
            if (overlap_length1 >= overlap_length2) {
                if (overlap_length1 >= deletion.length() / 2.0 ||
                    ...
                    pointer.previous();
                    pointer.insert(
                        Diff(EQUAL, insertion.left(overlap_length1)));
                    prevDiff->text =
                        deletion.left(deletion.length() - overlap_length1);
                    thisDiff->text = safeMid(insertion, overlap_length1);
                    // pointer.insert inserts the element before the cursor, so
                    // there is
                ...
                if (both_types) {
                    ...
                    if (commonlength != 0) {
                        if (pointer.hasPrevious()) {
                            thisDiff = &pointer.previous();
                            if (thisDiff->operation != EQUAL) {
                                throw "Previous diff should have been an "
                                      "equality.";
                            }
                            ...
                            pointer.insert(
                                Diff(EQUAL, text_insert.left(commonlength)));
                        }
    ...
    while (nextDiff != NULL) {
        if (prevDiff->operation == EQUAL && nextDiff->operation == EQUAL) {
            ...
            if (thisDiff->text.endsWith(prevDiff->text)) {
                ...
                thisDiff->text =
                    safeMid(thisDiff->text, nextDiff->text.length()) +
                    nextDiff->text;
                pointer.remove(); // Delete nextDiff.
                nextDiff = pointer.hasNext() ? &pointer.next() : NULL;
                changes = true;
...
QString diff_match_patch::diff_prettyHtml(const QList<Diff> &diffs)
{
    ...
    foreach (Diff aDiff, diffs) {
        ...
        switch (aDiff.operation) {
        case INSERT:
            html += QString("<ins style=\"background:#b9ff99;\">") + text +
                    QString("</ins>");
            break;
        case DELETE:
            html += QString("<del style=\"background:#ff4957;\">") + text +
                    QString("</del>");
            break;
        case EQUAL:
    ...
    foreach (Diff aDiff, diffs) {
        ...
        case INSERT: {
            QString encoded = QString(
                QUrl::toPercentEncoding(aDiff.text, " !~*'();/?:@&=+$,#"));
                                             ...
                                             const QString &delta)
{
    ...
    foreach (QString token, tokens) {
        ...
        case '=': {
            ...
            n = param.toInt();
            if (n < 0) {
                throw QString("Negative number in diff_fromDelta: %1")
                    .arg(param);
            }
            QString text;
...
void diff_match_patch::patch_addContext(Patch &patch, const QString &text)
{
    ...
    // Add the prefix.
    QString prefix =
        safeMid(text, std::max(0, patch.start2 - padding),
...
QPair<QString, QVector<bool>>
diff_match_patch::patch_apply(QList<Patch> &patches, const QString &sourceText)
{
    ...
    foreach (Patch aPatch, patchesCopy) {
        ...
        if (text1.length() > Match_MaxBits) {
            ...
            // a monster delete.
            start_loc =
                match_main(text, text1.left(Match_MaxBits), expected_loc);
            if (start_loc != -1) {
                end_loc =
                    match_main(text, text1.right(Match_MaxBits),
        ...
        if (start_loc == -1) {
            ...
            if (end_loc == -1) {
                ...
                text2 = safeMid(text, start_loc,
                                end_loc + Match_MaxBits - start_loc);
            }
            if (text1 == text2) {
                ...
                if (text1.length() > Match_MaxBits &&
                    diff_levenshtein(diffs) /
                            static_cast<float>(text1.length()) >
...
void diff_match_patch::patch_splitMax(QList<Patch> &patches)
{
    ...
    while (!bigpatch.isNull()) {
        ...
        while (!bigpatch.diffs.isEmpty()) {
            ...
            while (!bigpatch.diffs.isEmpty() &&
                ...
                if (diff_type == INSERT) {
                    ...
                    if (diff_text == bigpatch.diffs.front().text) {
                        ...
                        bigpatch.diffs.front().text = safeMid(
                            bigpatch.diffs.front().text, diff_text.length());
                    }
                }
            }
            ...
            precontext =
                safeMid(precontext, precontext.length() - Patch_Margin);
...
QList<Patch> diff_match_patch::patch_fromText(const QString &textline)
{
    ...
    while (!text.isEmpty()) {
        ...
        while (!text.isEmpty()) {
            ...
            line =
                line.replace("+", "%2B"); // decode would change all "+" to " "
            ...
            if (sign == '-') {
                ...
                // WTF?
                throw QString("Invalid patch mode '%1' in: %2")
                    .arg(sign)
                    .arg(line);
                return QList<Patch>();
            }
