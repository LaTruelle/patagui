...
{
    connect(manager, SIGNAL(valueChanged(QtProperty *, const QVariant &)), this,
            SLOT(slotPropertyChanged(QtProperty *, const QVariant &)));
    connect(manager, SIGNAL(attributeChanged(QtProperty *, const QString &,
                                             const QVariant &)),
            this, SLOT(slotPropertyAttributeChanged(
                      QtProperty *, const QString &, const QVariant &)));
                                      ...
                                      QtProperty *property, QWidget *parent)
{
    if (manager->propertyType(property) == VariantManager::filePathTypeId()) {
        ...
        editor->setFilter(
            manager->attributeValue(property, QLatin1String("filter"))
                .toString());
        theCreatedEditors[property].append(editor);
        theEditorToProperty[editor] = property;

        connect(editor, SIGNAL(pathChanged(const QString &)), this,
                SLOT(slotSetValue(const QString &)));
        connect(editor, SIGNAL(destroyed(QObject *)), this,
                SLOT(slotEditorDestroyed(QObject *)));
        return editor;
    } else if (manager->propertyType(property) ==
               VariantManager::unitTypeId()) {
        QSpinBox *spinbox = new QSpinBox(parent);
        spinbox->setValue(manager->value(property).toInt());
        // spinbox->setSuffix(manager->attributeValue(property,
        // QLatin1String("unit")).toString());
        // hardcoded as the solution above does not work and we only have one
        // type of spinbox (for font size)
        ...
        connect(spinbox, SIGNAL(valueChanged(int)), this,
                SLOT(slotSetIntValue(int)));
        connect(spinbox, SIGNAL(destroyed(QObject *)), this,
                SLOT(slotEditorDestroyed(QObject *)));
        return spinbox;
    }
...
void VariantFactory::disconnectPropertyManager(
...
{
    if (!theCreatedEditors.contains(property) &&
        !theCreatedSpinBoxes.contains(property))
        return;
